...
impl TargetCollector {
    ...
    pub(crate) fn run(path: PathBuf, parallel: bool) -> io::Result<Vec<PathBuf>> {
        ...
        let unprocessed = if parallel {
            entries
                .par_iter()
                .map(|e| Self::determine_target(e, parallel))
    ...
    fn determine_target(entry: &DirEntry, parallel: bool) -> io::Result<MaybeTarget> {
                ...
                .unwrap_or(false)
        {
            true => {
                ...
                match git_sub_directory.exists() && git_sub_directory.is_dir() {
                    ...
                    false => Ok(MaybeTarget::Multiple(Self::run(path, parallel)?)),
                }
            }
